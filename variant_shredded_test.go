...
// variant files and verifies that construct_variant reconstruction (see
// variant_shredded_read.go) produces the documented golden values. Before
// the shredded read path existed, these files were read back as nil/empty
// values without error, because parquet.Convert dropped the typed_value
// columns when converting to the reader's unshredded variant schema.
...
// shredded-to-unshredded schema substitution rebuilds the ancestor groups of
// the variant column; losing the ancestors' optional wrappers (or field IDs)
// in the rebuild shifts every definition level in the subtree and corrupts
// the read.
...
// non-generic Reader with an explicit unshredded schema. Both NewReader and
// the schema re-derivation in Reader.Read must apply the shredded-schema
// substitution, or the typed_value columns are dropped by the conversion and
// values silently read as nil.
...
// TestShreddedVariantBufferPath writes shredded variants through
// GenericBuffer (the column-buffer write path) rather than the row path, and
// round-trips objects, scalar fallbacks, and lists whose element values
// carry deeper repetition levels than the variant group itself.
func TestShreddedVariantBufferPath(t *testing.T) {
	shredded, err := parquet.ShreddedVariant(parquet.Group{
		"a": parquet.Int(64),
		"l": parquet.List(parquet.String()),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	rows := []row{
		{0, map[string]any{"a": int64(7), "l": []any{"x", "y", "z"}}},
		{1, "scalar fallback"},
		{2, map[string]any{"a": int64(8)}},
	}

	buffer := parquet.NewGenericBuffer[row](schema)
	if _, err := buffer.Write(rows); err != nil {
		t.Fatal(err)
	}
	out := new(bytes.Buffer)
	w := parquet.NewGenericWriter[row](out, schema)
	...
	got, err := parquet.Read[row](bytes.NewReader(out.Bytes()), int64(out.Len()))
	if err != nil {
		t.Fatalf("reading buffer-path shredded variant: %v", err)
	}
	if len(got) != 3 {
		t.Fatalf("read %d rows, want 3", len(got))
	}
	obj, ok := got[0].Var.(map[string]any)
	if !ok || obj["a"] != int64(7) {
		t.Fatalf("row 0 = %#v, want object with a=7", got[0].Var)
	}
	list, ok := obj["l"].([]any)
	if !ok || len(list) != 3 || list[0] != "x" || list[1] != "y" || list[2] != "z" {
		t.Errorf("row 0 l = %#v, want [x y z]", obj["l"])
	}
	if got[1].Var != "scalar fallback" {
		t.Errorf("row 1 = %#v, want scalar fallback", got[1].Var)
	}
	obj2, ok := got[2].Var.(map[string]any)
	if !ok || obj2["a"] != int64(8) {
		t.Errorf("row 2 = %#v, want object with a=8", got[2].Var)
	}
	if _, present := obj2["l"]; present {
		t.Errorf("row 2 l present = %#v, want missing field omitted", obj2["l"])
	}
}

// TestShreddedVariantRepeatedColumn round-trips a repeated shredded variant
// column (a list of variants). Every occurrence after the first starts at a
// non-zero repetition level, so this exercises the base repetition
// bookkeeping of both the shredding writer and construct_variant.
func TestShreddedVariantRepeatedColumn(t *testing.T) {
