...
func buildShreddedVariantGroup(node Node, startCol, defLevel, repDepth int) (*shreddedVariantGroup, error) {
	...
	for _, f := range node.Fields() {
		...
		case "metadata":
			...
			if !f.Leaf() || f.Type().Kind() != ByteArray || f.Optional() {
		...
		case "value":
			...
			// fields". The level arithmetic of the read and write paths
			// relies on the value column contributing one definition level.
			if !f.Optional() {
...
func (g *shreddedVariantGroup) read(r *variantColumnReader, m variant.Metadata, baseDef, baseRep int) (variant.Value, bool, error) {
	...
	case shreddedTypedObject:
		...
		if objPresent {
			...
			fields := append([]variant.Field{}, obj.ObjectValue().Fields...)
...
// variantAwareReadSchema returns the schema to use for reading rows of a
// file into values of the reader schema. When the reader schema declares an
// unshredded variant column but the file stores it shredded, the file's
// shredded group is substituted into the reader schema so its typed_value
// columns are reconstructed by construct_variant instead of being dropped
// by the column conversion (which would otherwise silently produce empty
// values).
func variantAwareReadSchema(readerSchema *Schema, fileSchema *Schema) *Schema {
	merged, changed := mergeVariantReadNodes(readerSchema, fileSchema)
	if !changed {
		return readerSchema
	}
	return NewSchema(readerSchema.Name(), merged)
}

func mergeVariantReadNodes(reader, file Node) (Node, bool) {
	if isVariant(reader) && isVariant(file) {
		if isUnshreddedVariant(reader) && !isUnshreddedVariant(file) {
			return file, true
		}
		return reader, false
	}
	if reader.Leaf() || file.Leaf() {
		return reader, false
	}
	changed := false
	group := make(Group)
	for _, readerField := range reader.Fields() {
		merged := Node(readerField)
		if fileField := fieldByName(file, readerField.Name()); fileField != nil {
			if m, ch := mergeVariantReadNodes(readerField, fileField); ch {
				merged, changed = m, true
			}
		}
		group[readerField.Name()] = merged
	}
	if !changed {
		return reader, false
	}
	// Rebuilding as a Group loses the reader node's own wrappers; restore
	// them so enclosing optional/repeated levels (and field IDs) survive
	// the substitution.
	var merged Node = group
	switch {
	case reader.Repeated():
		merged = Repeated(merged)
	case reader.Optional():
		merged = Optional(merged)
	}
	if id := reader.ID(); id != 0 {
		merged = FieldID(merged, id)
	}
	return merged, true
}
